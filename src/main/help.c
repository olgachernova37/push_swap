...
/*   Updated: 2025/03/07 16:40:17 by olcherno         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../push_swap.h"

void	lstadd_front(t_stack_node **lst, t_stack_node *new)// add new at the top
{
	if (lst && new)
	{
		new->next = *lst;
...
int	lst_size(t_stack_node *lst) // size of lists
{
	int counter;
	counter = 0;
	while (lst != NULL)
...
t_stack_node	*lst_last(t_stack_node *lst) // last list
{
	if (!lst)
		return (NULL);
	while (lst->next != NULL)
...
void	lstadd_back(t_stack_node **lst, t_stack_node *new) // add new in the end
{
	t_stack_node *last;
	if (NULL == lst)
		return ;
	...
	last = lst_last(*lst); // last element
	if (last->next = NULL)
	{
		last->next = new;
		new->next = NULL;
